...
are generated from the mounted test initial-condition tensor with a damped
Fourier autoregressive forecast: the first 20 observed frames are copied
exactly, per-frequency complex growth factors are estimated only from those
observed frames, magnitudes are clamped for stability, and the remaining frames
are rolled forward in Fourier space. Cylinder predictions use the mounted
inference script when available. It logs selected data paths, source decisions,
